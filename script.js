...
function showToast(message, type = 'success') {
    ...
    toast.innerHTML = `<i class="fas fa-${icon}" style="color:${color}; font-size: 1.2rem;"></i> ${message}`;
...
async function fetchAnnouncements() {
    try {
        ...
        if (data.length > 0) {
            ...
            // 2. Start Slider
            if (slider) {
                startAnnouncementSlider(slider, data);
            }
        } else {
    ...
    grid.innerHTML = visibleItems.map(item => {
        ...
        <div class="market-card" style="position:relative;">
            ...
            <div class="market-img">
                <img src="${item.image || 'images/default_market.jpg'}" alt="${item.title}" onerror="this.src='https://via.placeholder.com/400x300?text=Köy+Ürünü'">
                <span class="price-tag">${item.price}</span>
            </div>
            <div class="market-info">
                <h4>${item.title}</h4>
                <p class="seller"><i class="fas fa-user-circle"></i> ${item.phone || 'Satıcı'}</p>
